...
def test_export_single_component_single_course_json_no_pii(isolated_client):
...
def test_export_pii_requires_confirmation(isolated_client):
    ts = datetime(2026, 1, 15, 12, 0, tzinfo=timezone.utc).isoformat().replace("+00:00", "Z")
    _write_escalation("anth101", ts)
...
def test_export_zip_all_courses_split_by_course(isolated_client):
...
def test_export_custom_range_filters_by_local_date(isolated_client):
