...
3. **Validate instruction fit**
   - Update `backend/app/rag/prompts.py` to fine-tune model instructions for the new model, ensuring it honors citation formatting.
4. **Regression test**
   - Run `POST /ask` against a small set of smoke queries to confirm citation accuracy and confidence scoring.
