...
- backend/app/services/instructor_auth.py. Default demo secrets now rotate to a persisted random value and default credentials require explicit opt-in (A02/A07).
- tests/{conftest.py,test_api.py,test_escalation_security.py,test_retrieval.py,test_generate.py,test_escalations_store.py,test_resource_security.py,test_instructor_auth_security.py}. Coverage for fallbacks, validation, cleanup without `missing_ok`, resource traversal protection, and instructor auth secret/credential guards (regression guards).
